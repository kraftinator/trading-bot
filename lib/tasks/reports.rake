...
namespace :reports do
  ...
  task :activity => :environment do
    ...
    results << "#{'%-3s' % 'ID'}  #{'%-21s' % 'STRATEGY'}  #{'%3s' % 'BUY%'}   #{'%3s' % 'SELL%'}  #{'%3s' % 'CLG%'}  #{'%5s' % 'SELLS'}  #{'%6s' % 'TOTAL ' + trading_pair.coin.symbol}  #{'%20s' % 'LAST ACTION'}  #{'%5s' % 'SIDE'} #{'%10s' % 'PRICE'}"
    results << "--------------------------------------------------------------------------------------------------------------"
    ...
    bots.each do |bot|
      ...
      results << "#{'%-3s' % bot.id.to_s}  #{'%-21s' % bot.display_name}  #{'%.3f' % bot.buy_pct}  #{'%.3f' % bot.sell_pct}  #{'%.3f' % bot.ceiling_pct}  #{'%4s' % bot.sell_count}  #{'%.8f' % bot.coin_amount}  #{'%19s' % last_action_words }  #{'%4s' % side}  #{'%.8f' % price}"
